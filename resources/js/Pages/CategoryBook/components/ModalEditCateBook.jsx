...
import { svEditCateBook } from '@/services/author_publisher_illust/categorybook.service';
...
export default function ModalEditCateBook({open, handleClose, slcCateBook}) {
  const ImageRef = useRef([]);
  const [parentId, setParentId] = useState([]);
  const [imagePreview, setImagePreview] = useState("/image/no-image.png");
  const [title, setTitle] = useState(slcCateBook.title_cate);
  const [another, setAnother] = useState(slcCateBook.description_cate);
  ...
  const submit = () => {
    ...
    formData.append("title_cate", title);
    formData.append("description_cate", another);
    ...
    svEditCateBook(slcCateBook.id, formData).then((res) => {
      console.log(res)
      if(res.status == 'success') {
        // setCateBook(prevCateBook => [...prevCateBook, res.catebook]);
        handleClose()
      }
    })
  }
