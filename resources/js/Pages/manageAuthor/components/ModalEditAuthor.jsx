...
import { svEditAuthor } from '@/services/author_publisher_illust/author.service';
...
export default function ModalEditAuthor({open, handleClose, authorData}) {
  const ImageRef = useRef([]);
  const [parentId, setParentId] = useState([]);
  const [imagePreview, setImagePreview] = useState("/image/no-image.png");
  const [title, setTitle] = useState(authorData.author_name);
  const [another, setAnother] = useState(authorData.another_name);
  ...
  const submit = () => {
    ...
    svEditAuthor(authorData.id,formData).then((res) => {
      console.log(res)
