...
import { svEditIllustrator } from '@/services/author_publisher_illust/illust.service';
...
export default function ModalEditIllust({open, handleClose, illustData}) {
  const ImageRef = useRef([]);
  const [parentId, setParentId] = useState([]);
  const [imagePreview, setImagePreview] = useState("/image/no-image.png");
  const [title, setTitle] = useState(illustData.illust_name);
  const [another, setAnother] = useState(illustData.another_name);
  ...
  const submit = () => {
    ...
    svEditIllustrator(illustData.id,formData).then((res) => {
      console.log(res)
